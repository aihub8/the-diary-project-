...
const DiaryList = () => {
  ...
  const getDiaryList = async (user_id,page) => {
    return await axios
      .get(url.url + `/diary/${user_id}/getOtherList?page=${page}&perPage=6`, {
        headers: {
  ...
  const onClickPagination = (page) => {
    getDiaryList(cookies.userData.user_id,page);
  };
    ...
    <main className="diary__list">
      ...
      <div style={{ textAlign: "center" }}>
        ...
        >
          ...
          <ul class="pagination">
            {page.page-1<1?(<>첫페이지입니다</>):
    (<li class="page-item"><a class="page-link" onClick={()=>{onClickPagination(page.page-1)}}> {page.page-1}</a></li>)}
    <li class="page-item"><a class="page-link" >{page.page}</a></li>
    {page.page+1>page.totalPage?
    (<>마지막 페이지</>):<li class="page-item"><a class="page-link" onClick={()=>{onClickPagination(page.page+1)}}>{page.page+1}</a></li>}
  </ul>
        </nav>
      </div>
    </main>
